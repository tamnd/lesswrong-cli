// Package cli builds the lw command tree on top of the lesswrong library.
package cli
...
type App struct {
	client *lesswrong.Client
	cfg    lesswrong.Config
...
func Root() *cobra.Command {
	app := &App{cfg: lesswrong.DefaultConfig()}

	root := &cobra.Command{
		Use:   "lw",
		Short: "Browse LessWrong rationalist community posts",
		Long: `lw reads LessWrong through its public GraphQL endpoint.
...
lw is an independent tool and is not affiliated with LessWrong.`,
	...
	root.AddCommand(
		app.topCmd(),
		app.postsCmd("new", "Most recent LessWrong posts", "new", 20),
		app.postsCmd("best", "Best LessWrong posts of all time", "best", 20),
		app.searchCmd(),
		app.postCmd(),
		newVersionCmd(),
	)
...
func (a *App) setup() error {
	...
	a.client = lesswrong.NewClient(a.cfg)
	return nil
}
